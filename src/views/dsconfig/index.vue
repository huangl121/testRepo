            ...
            <el-form-item prop="dbType">
                <el-select v-model="searchMap.dbType" placeholder="数据源类型" style="width: 120px">
                    ...
                    <el-option
                            ...
                            :key="option.db_type"
                            :label="option.name"
                            :value="option.db_type"></el-option>
                </el-select>
            </el-form-item>
...
<!--            </el-form-item>-->
      ...
      -->
        <el-table :data="list" height="380" border style="width: 100%">
            ...
            <el-table-column type="dsId" label="数据源主码" width="100"></el-table-column>
            <el-table-column prop="dbName" label="名称" width="120"></el-table-column>
            <!-- 数据源类型开始 -->
            <el-table-column prop="dbType" label="类型">
                <template slot-scope="scope">
                    <!-- 使用过滤器,scope.row.pay_type为接口返回的pay_type的值，后面的是作用在哪个过滤器上面，这里作用在 payTypeFilter 上面-->
                    <span>{{scope.row.db_type | dbTypeFilter }}</span>
                </template>
            </el-table-column>
            <!-- 数据源类型结束 -->

            <el-table-column prop="addIP" label="URL" width="100"></el-table-column>
            <el-table-column prop="port" label="端口" width="120"></el-table-column>
            <el-table-column prop="user" label="登录用户"></el-table-column>
...
<script>
    import dsApi from "@/api/dsconfig"
    ...
    export default{
        data(){
            return {
                ...
                dbTypeOptions, // 这里要申明，要不然搜索框里使用的时候会报错
        ...
        updateData(formName) {
            ...
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    // 提交更新
                    dsApi.update(this.pojo).then(response => {
                        const res = response.data;
                        if (res.success) {
                            // 刷新列表
                            this.fetchData();
        ...
        filters: {
            ...
            dbTypeFilter(db_type) {
                const dbObj = dbTypeOptions.find(obj => {
                    // obj 就是上面的一个对象，type和上面对象的type进行对比，在返回
                    return obj.db_type === db_type;
                });
                // payObj 有值，返回 payObj.name
                return dbObj ? dbObj.name : null; // 没找到类型返回null，找到了返回payObj.name
            }
        }
    }
</script>
